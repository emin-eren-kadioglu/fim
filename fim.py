...
IGNORED_EXTENSIONS = {
    ...
    ".o", ".obj", ".pdb", ".idb", ".ipch", ".sqlite", ".db-journal"
}
...
def init_db():
    ...
    try:
        with sqlite3.connect(LOG_FILE_DATA) as conn:
            cursor = conn.cursor()
            cursor.execute('''
        ...
        log_system("CRITICAL", "DB_INIT", "Could not initialize database", str(e))
...
# Diskin kullanılabilir olup olmadığını anlayan fonksiyon.
...
# Dosyanın kimin olduğunu bulan fonksiyon.
def get_owner(path):
    try:
...
# Dosyanın metadatasını çeken fonksiyon.
                ...
                with CSV_LOCK: 
                    with sqlite3.connect(LOG_FILE_DATA) as conn:
                        ...
                        cursor.execute("""
                            ...
                            (id, date, path, type, message, event, created, accessed, modified, size, machine, ip, owner)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                        ...
                        conn.commit()
                break 
            except sqlite3.OperationalError: # Veritabanı kilitli ise bekle
                time.sleep(0.5)
                retries -= 1
            except Exception:
                break
...
# Aktif olan explorer penceresini yakalar.
...
# Dizin erişmini kontrol eder ve dizinin altındaki ilk 50 dosyayı loglar.
    ...
    try:
        while True:
            ...
            if current and current != last_path:
                if current.upper().startswith(SYSTEM_DRIVE):
